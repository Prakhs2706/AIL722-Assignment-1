...
def save_policy_gif(policy, filename, envr=FootballSkillsEnv, horizon=40):
    ...
    if isinstance(policy, np.ndarray) and policy.ndim==1:
        policy=np.tile(policy, (horizon, 1))  # make it 2D for time-dependent so that it accepts
...
if __name__=="__main__":
    ...
    print("Total transition calls in Vanilla Value Iteration:", transition_call_counter["vi"])
